...
1. Obtained data from Twitter API
...
# punctuations, remove stop words, and strip white space
...
```{r}
# Examine terms most associated with each of the 5 topics in a table
top_terms <- lda_betas %>%
  group_by(topic) %>%
  top_n(10, beta) %>%
  ungroup() %>%
...
# Examine terms most associated with each of the 5 topics in a graph
top_terms %>%
...
1. Includes one, can, time, but also last - I'm thinking this one has to do with 
people either going back and rewatching the series from season one, or beginning 
the show, possibly as a result of the shut-down leading to more free time

2. This one is tough - maybe just tweets about content from people watching

3. This one has "jon", "king", and also "remember" - I'm thinking hopefuls who
think Jon will end up on the throne
...
Given all the NLP you’ve done and analyses you’ve conducted, would you conclude that topics (and
emotion) are important in predicting the popularity of tweets? Why or why not? Cite specific
evidence from your earlier work on this project and also consider the research design and the
impact of how topic modeling works carefully
